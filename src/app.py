...
from auth.password_reset import show_password_reset
...
def init_session():
    pass  # Si necesitas inicializar algo en session_state, hazlo aquí
...
if not is_authenticated():
    ...
    # Renderizar la pantalla correcta
...
try:
    ...
    with st.sidebar:
        ...
        user_role = st.session_state.get('user_role', 'paciente')
        st.markdown(f"""
            <div style='text-align:center; margin-bottom:0.5rem;'>
                <b>{user_name}</b><br>
                <span style='color:gray; font-size:0.9rem;'>{user_role.title()}</span>
            </div>
        """, unsafe_allow_html=True)
        ...
        if st.button("🔄 Actualizar datos", key='btn_actualizar'):
            update_data()
        st.divider()
        # Selector de usuario (solo para médico/administrador)
        if user_role in ['medico', 'administrador']:
